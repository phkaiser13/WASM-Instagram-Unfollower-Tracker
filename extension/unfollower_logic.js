 ...
 * @param {any} new_followers_js
 * @param {Uint8Array} old_followers_mpack
 * @returns {any}
 */
export function find_unfollowers(new_followers_js, old_followers_mpack) {
    const ptr0 = passArray8ToWasm0(old_followers_mpack, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.find_unfollowers(new_followers_js, ptr0, len0);
    if (ret[2]) {
 ...
 *   serialization fails.
 * @param {any} followers_js
 * @returns {Uint8Array}
 */
export function serialize_followers_to_mpack(followers_js) {
    const ret = wasm.serialize_followers_to_mpack(followers_js);
    if (ret[3]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('unfollower_logic_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
